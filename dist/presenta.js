// https://lib.presenta.cc v0.1.19 - BSD-3-Clause License - Copyright 2021 Fabio Franchino
(function (global, factory) {
  ...
  */
  ...
  const arrows = function (rootElement, router, ctrlConfig, projectConfig) {
    ...
    const vertical = ctrlConfig.direction === 'vertical' ? css$m.vertical : '';
    ...
    const child = utils.div(`<div class="${css$m.arrows} ${vertical}"></div>`);
    const inner = utils.div(`<div class="${css$m.inner}" style="${rawp}"></div>`);
    const left = utils.div(`<div id="evt_trg_ctrl_arrow_left" class="${css$m.left}">
    <div class="${css$m.ui}">${firstArrow}</div>
  </div>`);
    inner.appendChild(left);
    const right = utils.div(`<div id="evt_trg_ctrl_arrow_right" class="${css$m.right}">
    <div class="${css$m.ui}">${lastArrow}</div>
  </div>`);
    ...
    const scheduleForHide = () => {
      clearTimeout(timer);
      child.classList.remove(css$m.hide);
      timer = setTimeout(() => {
        child.classList.add(css$m.hide);
      }, 1500);
    }; // scheduleForHide()
  ...
  var css_248z$r = ".style_black__3Nszx{width:100%;height:100%;position:absolute;top:0;left:0;background-color:#000;opacity:0;pointer-events:none;transition:opacity .5s cubic-bezier(.8,.2,.2,.8);z-index:999999}";
  var css$l = {"black":"style_black__3Nszx"};
  styleInject(css_248z$r);

  const black = function (rootElement, router, ctrlConfig, projectConfig) {
    let visible = false;
    const child = utils.div(`<div class="${css$l.black}"></div>`);
    rootElement.appendChild(child);
  ...
  const progressbar = function (rootElement, router, ctrlConfig, projectConfig) {
    ...
    const child = utils.div(`<div class="${css$k.progressbar}"></div>`);
    const bar = utils.div(`<div class="${css$k.bar}"></div>`);
  ...
  var css_248z$p = ".style_limitswitch__3g8Lq{pointer-events:none;opacity:0;background-color:var(--colorAccent);width:100%;height:100%;position:absolute;top:0;left:0}.style_signalSet__GRA42{opacity:.75}.style_signalOut__2qd5k{transition:opacity .35s ease-out;opacity:0}";
  var css$j = {"limitswitch":"style_limitswitch__3g8Lq","signalSet":"style_signalSet__GRA42","signalOut":"style_signalOut__2qd5k"};
  styleInject(css_248z$p);

  const limitswitch = function (rootElement, router, ctrlConfig, projectConfig) {
    let timer1;
    let timer2;
    const child = utils.div(`<div class="${css$j.limitswitch}"></div>`);
    rootElement.appendChild(child);

    const signal = () => {
      clearTimeout(timer1);
      clearTimeout(timer2);
      child.classList.remove(css$j.signalSet, css$j.signalOut);
      child.classList.add(css$j.signalSet);
      timer1 = setTimeout(() => {
        child.classList.add(css$j.signalOut);
      }, 16);
      timer2 = setTimeout(() => {
        child.classList.remove(css$j.signalSet, css$j.signalOut);
      }, 350);
  ...
  var css_248z$o = ".style_sync__2HAFR{width:100%;height:100%;position:absolute;top:0;left:0;pointer-events:none}";
  var css$i = {"sync":"style_sync__2HAFR"};
  ...
  const sync = function (rootElement, router, ctrlConfig, projectConfig) {
    ...
    const child = utils.div(`<div class="${css$i.sync}"></div>`);
    rootElement.appendChild(child);
  ...
  var css_248z$m = ".transition__hSlide .p-scene-enter-transition{transition:transform .75s cubic-bezier(.87,0,.13,1)}.transition__hSlide .to-right.p-scene-enter-start{transform:translateX(100%)}.transition__hSlide .to-right.p-scene-enter-end{transform:translateX(0)}.transition__hSlide .to-left.p-scene-enter-start{transform:translateX(-100%)}.transition__hSlide .to-left.p-scene-enter-end{transform:translateX(0)}.transition__hSlide .p-scene-leave-transition{transition:transform .75s cubic-bezier(.87,0,.13,1)}.transition__hSlide .to-right.p-scene-leave-start{transform:translateX(0)}.transition__hSlide .to-right.p-scene-leave-end{transform:translateX(-100%)}.transition__hSlide .to-left.p-scene-leave-start{transform:translateX(0)}.transition__hSlide .to-left.p-scene-leave-end{transform:translateX(100%)}";
  styleInject(css_248z$m);

  var css_248z$l = ".transition__vSlide .p-scene-enter-transition{transition:transform .75s cubic-bezier(.87,0,.13,1)}.transition__vSlide .to-right.p-scene-enter-start{transform:translateY(100%)}.transition__vSlide .to-right.p-scene-enter-end{transform:translateY(0)}.transition__vSlide .to-left.p-scene-enter-start{transform:translateY(-100%)}.transition__vSlide .to-left.p-scene-enter-end{transform:translateY(0)}.transition__vSlide .p-scene-leave-transition{transition:transform .75s cubic-bezier(.87,0,.13,1)}.transition__vSlide .to-right.p-scene-leave-start{transform:translateY(0)}.transition__vSlide .to-right.p-scene-leave-end{transform:translateY(-100%)}.transition__vSlide .to-left.p-scene-leave-start{transform:translateY(0)}.transition__vSlide .to-left.p-scene-leave-end{transform:translateY(100%)}";
  styleInject(css_248z$l);

  var css_248z$k = ".transition__slideOver .p-scene-enter-transition{transition:all .75s cubic-bezier(.2,.8,.05,.95)}.transition__slideOver .p-scene-enter-start{transform:translateY(-150%)}.transition__slideOver .p-scene-enter-end{transform:translateY(0)}.transition__slideOver .p-scene-leave-transition{transition:all .75s cubic-bezier(.2,.8,.05,.95)}.transition__slideOver .p-scene-leave-start{transform:scale(1)}.transition__slideOver .p-scene-leave-end{transform:scale(.5)}";
  ...
  */
  ...
  const steps = function (sceneElement, modConfig, sceneConfig) {
    ...
    for (const k in allElems) {
      ...
      allElems[k].arr.forEach(el => el.classList.add(css$h[trans]));
      const id = {
        sandbox: 'steps',
        index,
        trans,
    ...
    setTimeout(() => {
      ...
      allFlatElems.forEach(el => {
        el.classList.add(css$h.stepItem);
      }); // routine only for match mode

      for (const k in allElems) {
        allElems[k].arr.forEach(el => el.classList.add(css$h.stepItem));
      }
    }, 100);
      ...
      if (step.sandbox === 'steps') {
        if (prevEls) {
          prevEls.forEach((el, i) => {
            ...
            if (out) el.classList.add(css$h[step.trans]);
          });
          prevEls = null;
        }

        const els = step.els;
        els.forEach(el => el.classList.remove(css$h[step.trans]));
  ...
  var css_248z$i = "[jump]{cursor:pointer}";
  ...
  var css_248z$h = ".noresize.sceneObject{width:100%;height:100%}.noresize.sceneObject .sceneWrapper{width:100%;height:100%;transform:scale(1)}";
  ...
  */
  ...
  const props$6 = ['opacity', 'blend', 'radius', 'border', 'padding', 'background', 'color', 'clip', 'shadow'];
  ...
  */
  ...
  const props$5 = ['top', 'left', 'width', 'height', 'angle', 'right', 'bottom', 'skew'];
  ...
  var css_248z$f = ".style_reveal__3Cs6V{--revealTextAlign:center;--revealPadding:5px;--revealFontSize:2rem;--revealFont:var(--fontText);--revealBackColor:var(--colorFore);--revealColor:var(--colorBack);--revealFlexAlign:center;--revealFlexJustify:center;width:100%;height:100%;position:absolute;top:0;left:0;pointer-events:none;transition-property:opacity,transform;transition-duration:1.33s;transition-timing-function:cubic-bezier(.19,1,.22,1)}.style_inner__3rqnR{width:100%;height:100%;padding:var(--revealPadding);background-color:var(--revealBackColor);display:flex;align-items:var(--revealFlexAlign);justify-content:var(--revealFlexJustify)}.style_content__bTAxU{text-align:var(--revealTextAlign);font-size:var(--revealFontSize);color:var(--revealColor);font-family:var(--revealFont)}.style_preview__2jDoF{opacity:.3}.style_exitSlideUp__s74jg{transform:translateY(-100%)}.style_exitFadeOut__12FkH{opacity:0}";
  var css$f = {"reveal":"style_reveal__3Cs6V","inner":"style_inner__3rqnR","content":"style_content__bTAxU","preview":"style_preview__2jDoF","exitSlideUp":"style_exitSlideUp__s74jg","exitFadeOut":"style_exitFadeOut__12FkH"};
  styleInject(css_248z$f);

  const props$3 = ['textAlign', 'padding', 'fontSize', 'font', 'backColor', 'color', 'flexAlign', 'flexJustify'];

  const reveal = function (element, mod, config) {
    let isPreview = '';
    if (config._mode === 'preview') isPreview = css$f.preview;
    const rawp = utils.rawProps('reveal', props$3, mod);
    ...
    const exitEffect = mod.effect ? css$f[mod.effect] : css$f.exitSlideUp;
    const child = utils.div(`<div class="${css$f.reveal} ${isPreview}"></div>`);
    const inner = utils.div(`<div class="${css$f.inner}" style="${rawp}"></div>`);
    const content = utils.div(`<div class="${css$f.content}">${text}</div>`);
    inner.appendChild(content);
    child.appendChild(inner);
    element.querySelector('.sceneWrapper').appendChild(child);
  ...
  const enters = function (element, mod, config) {
    ...
    if (config.contextType === 'block') {
      ...
      el.classList.add(css$e[transition]);
      setTimeout(() => {
        el.classList.add(css$e.entersItem);
      }, 16);
      setTimeout(() => {
        el.classList.remove(css$e[transition]);
      }, delay);
    }
  }; // enters.runBefore = true

  var css_248z$d = ".style_viewportLimit__2yPva{width:100%;height:100%;position:absolute;border:3px dashed red;pointer-events:none;visibility:hidden}";
  var css$d = {"viewportLimit":"style_viewportLimit__2yPva"};
  styleInject(css_248z$d);

  function fabricbridge(element, modConfig, config) {
    if (config.contextType === 'block') return;
    const viewport = element.querySelector('.blocksContainer');
    const limits = utils.div(`<div class="${css$d.viewportLimit}"></div>`);
...
`;
  ...
  const text = function (el, config) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const rawp = utils.rawProps('text', props$2, config);
      let fsize = config.scale;
      const autoscale = config.autoscale;
      let ccss = '';
      const uid = utils.uid(config);
      ...
      const child = utils.div(`<div class="c ${uid} ${css$c.text} ${css$c.promise}">
    ${ccss}
    <div style="${rawp}" class="${css$c.inner}">
      <div class="pretext ${css$c.pretext}">
        <div class="${css$c.textbox}">
          <div class="textContent ${css$c.itext} ${css$c.fadein}">
            ${html}
          </div>
        </div>
      </div>
      ...
      const compute = () => {
        ...
        const mel = child.querySelector('.' + css$c.inner);
        const el = child.querySelector('.' + css$c.textbox);

        if (!autoscale) {
          setTimeout(() => {
            child.classList.remove(css$c.promise);
            resolve(that);
          });
          return false;
        }
  ...
  text.init = () => {// u.addGlob(['textVar', 'textStyle'])
  ...
  const embed = function (el, config) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const done = () => {
        ...
        child.querySelector('.' + css$b.poster).style.display = 'none';
        resolve(that);
      };
  ...
  const props$1 = ['scale', 'filter', 'position', 'size'];
  ...
  const image = function (el, config) {
    ...
    const that = this;
    return new Promise((resolve, reject) => {
      const rawp = utils.rawProps('image', props$1, config);
      const imageschunk = `<div class="presentablock__image ${css$a.preimg}"></div>`;
      const child = utils.div(`<div class="${css$a.image}">
    <div style="${rawp}" class="imagesContainer ${css$a.inner}">
        ${imageschunk}
    </div>
  </div>`);
      el.appendChild(child);
      const wrapper = el.querySelector('.' + css$a.preimg);
      const img = new Image();
  ...
  const video = function (el, config) {
    ...
    const child = utils.div(`<div class="${css$9.video}" id="evt_trg_uid_block_video_${config._index}">
  ...
  */
  ...
  const svg = function (el, config) {
    ...
    const child = utils.div(`<div class="c ${css$8.svg}">
  ...
  const shape = function (el, config) {
    ...
    const tEl = css$7[t];
    const child = utils.div(`<div class="${css$7.shape}">
    <div style="${rawp}" class="${css$7.el} ${tEl}"></div>
  </div>`);
    el.appendChild(child);
  };
  ...
  const line = function (el, config) {
    ...
    const dircls = css$6[dir];
    const child = utils.div(`<div class="${css$6.line} ${dircls}">
    <div style="${rawp}" class="${css$6.el}"></div>
  </div>`);
    el.appendChild(child);
  };
  ...
  const props = ['background', 'color', 'accent', 'padding', 'lineHeight', 'spaceChar', 'borderTop', 'borderLeft', 'borderRight', 'borderBottom', 'borderRadius', 'shadow', 'blend', 'align', 'vertical', 'horizontal'];
  ...
  const gtext = function (el, config) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let rawp = utils.rawProps('text', props, config);
      let fsize = config.scale;
      const autoscale = config.autoscale;
      let inlineStyleClasses = '';
      inlinestyles.forEach(s => {
        if (config[s]) {
          inlineStyleClasses += css$5[s] + ' ';
        }
      });
    ...
    <div style="${rawp}" class="${css$5.inner}">
      <div class="pretext ${css$5.pretext}">
        <div class="${css$5.textbox}">
          <div class="textContent ${css$5.itext} ${clampClass}">
            <span>${txt}</span>
          </div>
        </div>
      </div>
      ...
      const compute = () => {
        child.style.setProperty('--textSize', `${fsize}rem`);
        const mel = child.querySelector('.' + css$5.inner);
        const el = child.querySelector('.' + css$5.textbox);

        if (!autoscale) {
          setTimeout(() => {
            child.classList.remove(css$5.promise);
            resolve(that);
          });
          return false;
        }
